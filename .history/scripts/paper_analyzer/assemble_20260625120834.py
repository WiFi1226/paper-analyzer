...
def _run(args: argparse.Namespace, config: Config) -> None:
    ...
    if not orchestrator_json_path.exists():
        logger.error("编排器输出 JSON 不存在 —— %s", orchestrator_json_path)
        sys.exit(1)
    ...
    if sections_path is None:
        cache_dir = orchestrator_json_path.parent
        logger.error("无法定位 sections.json")
        logger.error("   已尝试:")
        logger.error("   1. orchestrator JSON 中的 sections_path 字段")
        logger.error("   2. %s", cache_dir / f"{paper_name}_sections.json")
        logger.error("   3. %s（目录结构推断）", cache_dir / f"{cache_dir.parent.name}_sections.json")
        sys.exit(1)

    with open(sections_path, encoding="utf-8") as f:
        sections: list[dict[str, Any]] = json.load(f)

    agent_outputs, cleanup_parts = load_agent_outputs(paper_name, args.outputs_dir, config)

    if not agent_outputs:
        logger.warning("未找到任何 agent 输出文件")
        logger.warning("   预期位置: %s", config.outputs_dir(paper_name))
    ...
    paper_dir: Path = config.output_dir / paper_name
