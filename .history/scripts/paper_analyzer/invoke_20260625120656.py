...
def _load_rules_from_disk() -> list[tuple[str, Path]]:
    """自动扫描 .github/instructions/ 目录下所有 .md 文件，按文件名排序。
    ...
    if not _RULES_DIR.exists():
        return []
    files = sorted(
        (f.name, f) for f in _RULES_DIR.iterdir()
        if f.is_file() and f.suffix == ".md"
    )
    return files


def load_rules() -> tuple[str, list[str]]:
    """按顺序读取 .github/instructions/*.md 并拼接为通用规则块。
    ...
    parts = []
    missing = []
    for fname, fpath in _load_rules_from_disk():
        if fpath.exists():
            parts.append(fpath.read_text(encoding="utf-8").strip())
        else:
            msg = f"规则文件缺失: {fname}"
            missing.append(msg)
            print(f"⚠️  {msg}（预期路径: {fpath}）", file=sys.stderr)
    return "\n\n".join(parts), missing
...
def _run_prompts(
    ...
    缓存 key = (agent, 该 agent 负责的全部章节标题集合)。
    每个 agent 的缓存条目包含若干 parts（拆分后的 prompt 文件）。
    """
    rules, missing_rules = load_rules()
    ...
    # 从 agents.yaml 获取需要注入完整章节标题列表的 agent 集合
    agents_cfg = load_agents().get("agents", {})
    ...
    for agent, titles in agent_titles.items():
        ...
        if cached_agent and is_agent_entry_fresh(
            ...
            if restored_parts is not None:
                ...
                print(f"[缓存] prompt 缓存命中（agent={agent}, {len(titles)} 个章节）", file=sys.stderr)
                continue
            # 文件缺失 → 降级为重新生成
            warnings.append(
                f"缓存文件缺失（agent={agent}），将重新生成该 agent 的所有 prompt"
            )
...
def main() -> None:
    ...
    if not sections_path.exists():
        print(f"错误：sections JSON 不存在 —— {sections_path}", file=sys.stderr)
        sys.exit(1)

    routing_path = Path(args.routing_json)
    if not routing_path.exists():
        print(f"错误：routing JSON 不存在 —— {routing_path}", file=sys.stderr)
        sys.exit(1)
    ...
    result = _run_prompts(sections, matches, paper_name, cache_dir, max_chars)
