...
def load_agent_outputs(
    paper_name: str, outputs_dir_override: str | None = None,
) -> tuple[dict[str, str], list[Path]]:
    """读取所有 agent 输出，自动合并拆分的分片。

    优先从 outputs_dir_override 读取，否则回退到 output/<论文名>/cache/_outputs/。

    如果存在 <agent>_part1.md、<agent>_part2.md 等分片文件，
    自动按序号拼接（以 --- 分隔），合并写入 <agent>.md。
    分片文件**不**在此函数中删除——由调用方在报告保存成功后统一清理。
    ...
    if outputs_dir_override:
        ...
        outputs_dir = get_outputs_dir(paper_name)
    ...
    for agent_name, paths in part_files.items():
        if len(paths) <= 1:
            # 单分片无意义（可能是旧缓存残留），跳过合并但告警
            if paths:
                print(f"⚠️  告警：agent={agent_name} 仅有单个分片文件（非拆分场景），忽略 —— {paths[0].name}",
                      file=sys.stderr)
            continue
        ...
        for p in paths:
            sz = p.stat().st_size
            if sz == 0:
                print(f"⚠️  告警：分片文件为空 —— {p}", file=sys.stderr)
            elif sz < 100:
                print(f"⚠️  告警：分片文件异常小（{sz} 字节）—— {p}", file=sys.stderr)
        ...
        if merged_len < len(paths) * 500:
            print(f"⚠️  告警：合并后内容偏短 —— agent={agent_name}"
                  f"（{merged_len} 字符，{len(paths)} 个分片，"
                  f"预期至少 {len(paths) * 500} 字符）",
                  file=sys.stderr)
