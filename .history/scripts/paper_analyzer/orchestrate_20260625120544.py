...
def resolve_input(user_input: str) -> tuple[Path, Path | None]:
    """解析用户输入，返回 (txt_path, pdf_path_or_none)。

    支持三种输入形式：
      - /path/to/file.txt — 直接使用，pdf_path=None
      - /path/to/file.pdf — 转换并缓存，返回 (txt_path, pdf_path)
      - 仅文件名 — 在 output/<name>/cache/ 下查找，尝试同名 PDF

    Returns:
        (txt_path, pdf_path)
        pdf_path 为 None 表示输入不是 PDF（或无法追溯到原始 PDF）。
    """
    ...
    if input_path.suffix.lower() == ".pdf":
        if not input_path.exists():
            ...
            sys.exit(1)
        txt_path, _ = _convert_pdf(input_path)
        return txt_path, input_path

    # 情况 3: 用户仅给文件名 —— 在 output/<name>/cache/ 下查找
    stem = _normalize_paper_name(input_path.stem)
    cached_txt = get_txt_path(stem)
    if cached_txt.exists():
        pdf_path = _find_original_pdf(cached_txt)
        return cached_txt, pdf_path

    # 在 pdf_search_paths 配置的目录中查找同名 PDF
    settings = load_settings()
    search_paths = settings.get("pdf_search_paths") or []
    tried = [input_path, cached_txt]
    for sp in search_paths:
        candidate = get_project_root() / sp / f"{stem}.pdf"
        tried.append(candidate)
        if candidate.exists():
            txt_path, _ = _convert_pdf(candidate)
            return txt_path, candidate

    # 都没找到
    print(f"错误：无法找到文件 '{user_input}'", file=sys.stderr)
    print(f"   已尝试: {', '.join(str(t) for t in tried)}", file=sys.stderr)
    sys.exit(1)


def _find_original_pdf(txt_path: Path) -> Path | None:
    """尝试在 pdf_search_paths 配置的目录中查找 txt 对应的原始 PDF。

    用于纯文件名的输入场景--用户只给了文件名，系统在 output/<name>/cache/
    下找到了已有的 txt，此时尝试追溯到原始 PDF 以便后续 mtime 判断。
    """
    settings = load_settings()
    search_paths = settings.get("pdf_search_paths") or []
    paper_name = txt_path.stem
    for sp in search_paths:
        candidate = get_project_root() / sp / f"{paper_name}.pdf"
        if candidate.exists():
            return candidate
    return None
...
def _convert_pdf(pdf_path: Path) -> tuple[Path, str]:
    """PDF → TXT，基于文件夹内 mtime 判断。

    流程：
      1. 检查 output/<paper>/cache/<paper>.txt 是否存在且比 PDF 新
      2. 新鲜 → 直接复用
      3. 不新鲜或不存在 → pdftotext 提取 → 保存

    Returns:
        (txt_path, cache_status)  cache_status: "hit" 或 "miss"
    """
    paper_name = _normalize_paper_name(pdf_path.stem)
    txt_path = get_txt_path(paper_name)

    # ── 缓存判断：txt 存在且 mtime 不早于 PDF → 复用 ──
    if txt_path.exists() and txt_path.stat().st_mtime >= pdf_path.stat().st_mtime:
        print(f"[缓存] 文本缓存新鲜，跳过提取: {txt_path}", file=sys.stderr)
        return txt_path, "hit"

    # ── 首次提取或 PDF 已更新 ──
    if not check_pdftotext():
        print("错误：未找到 pdftotext 命令，请安装 poppler-utils", file=sys.stderr)
        sys.exit(1)

    print(f"[PDF] 检测到 PDF，正在提取文本: {pdf_path.name}", file=sys.stderr)
    try:
        text = extract_text(pdf_path)
    except RuntimeError as e:
        print(f"错误：PDF 提取失败 —— {e}", file=sys.stderr)
        sys.exit(1)

    if not text.strip():
        print("警告：提取到的文本为空（可能是扫描版 PDF）", file=sys.stderr)
    ...
    lines = text.count("\n") + 1
    print(f"[完成] 文本提取完成 → {txt_path}（{lines:,} 行，{len(text):,} 字符）", file=sys.stderr)
