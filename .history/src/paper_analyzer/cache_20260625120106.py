...
def load_existing_meta(cache_dir: Path) -> dict[str, Any] | None:
    """读取已有缓存元数据。返回 None 表示无缓存或格式不兼容。"""
    prompts_dir = cache_dir / "_prompts"
    meta_path = prompts_dir / "_meta.json"
...
def collect_kept_files(agent_entry: dict[str, Any]) -> set[str]:
    ...
    for part in agent_entry.get("parts", []):
        ...
        if part.get("_content_path"):
            paths.add(part["_content_path"])
    return paths


def cleanup_orphaned_files(cache_dir: Path, kept_paths: set[str]) -> None:
    """删除 _prompts/ 目录中不被引用的孤儿 prompt/content 文件。"""
    prompts_dir = cache_dir / "_prompts"
    if not prompts_dir.exists():
        return
    for pattern in ["prompt_*.txt", "content_*.txt"]:
        for fpath in prompts_dir.glob(pattern):
            if str(fpath.resolve()) not in kept_paths:
                try:
                    fpath.unlink()
                except OSError:
                    pass
