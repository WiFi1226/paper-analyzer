...
def paper_content_formatter(
    ...
    if needs_chapter_list:
        ...
        footer_parts.extend(["", "", title_list])
...
def build_single_prompt(
    ...
    if len(titles) == 1:
        warnings.append(
            f"章节「{titles[0]}」的 prompt 为 {len(prompt_text):,} 字符，"
            f"超过上限 {max_chars:,}，无法拆分，章节内容将写入独立文件供 agent Read"
        )
        entry = _make_prompt_dict(agent, prompt_text, titles)
        entry["content_text"] = parts["content_block"]
        entry["skeleton_text"] = parts["skeleton"]
        entry["footer_text"] = parts["footer"]
        return [entry]
    ...
    left = build_single_prompt(
        agent, left_titles, rules, content_block, footer, max_chars, warnings,
    )
    right = build_single_prompt(
        agent, right_titles, rules, content_block, footer, max_chars, warnings,
    )
