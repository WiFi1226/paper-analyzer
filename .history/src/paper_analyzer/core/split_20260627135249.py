...
用法:
    ...
    if style == "cn":
        ...
        headings = find_en_headings(text, en_line_pattern, en_mid_pattern, heading_number_regex, dedup_distance=30)
...
# PDF 转文本后标题续行的最小缩进空格数
_CONTINUATION_INDENT = 4
# 续行文本最大长度（字符），超过此长度视为段落正文而非标题续行
_CONTINUATION_MAX_CHARS = 50


def _expand_continuation_lines(
    text: str,
    headings: list[tuple[int, int, str]],
    heading_number_regex: re.Pattern[str],
) -> list[tuple[int, int, str]]:
    """将标题后缩进续行合并到标题中（处理 PDF 换行截断）。

    PDF 转文本后，长标题如：
        VII. Additional Analyses: Economic Mechanisms and
                 Relationship Formation
    会被正则截断为 "VII. Additional Analyses: Economic Mechanisms and"。
    此函数检测标题 end_pos 后的缩进行，若存在则追加到标题文本。

    Args:
        text:                 论文全文
        headings:             原始标题列表 [(start, end, title), ...]
        heading_number_regex: 标题编号前缀正则（来自 YAML），用于检测续行是否为新标题
    ...
    for start, end, title in headings:
        ...
        while pos < len(text):
            ...
            # 检查后续行是否以缩进开头
            cont_match = re.match(r"\n {" + str(_CONTINUATION_INDENT) + r",}(\S.+)", rest)
            if not cont_match:
                break
            cont_text = cont_match.group(1).strip()
            # 续行本身以标题编号开头 → 新标题，不合并
            if heading_number_regex.match(cont_text):
                break
        ...
        if lines_collected:
            joined = " ".join(lines_collected)
            expanded.append((start, pos, title + " " + joined))
        else:
            expanded.append((start, end, title))
...
def find_en_headings(
    ...
    自动将换行截断的标题续行（缩进 4+ 空格）合并回标题文本。

    Args:
        text:                 论文全文
        en_line:              英文行首模式正则
        en_mid:               英文中继模式正则
        heading_number_regex: 标题编号前缀正则（来自 YAML），用于续行检测
        dedup_distance:       去重距离阈值（字符数）。

    Returns:
        [(start_pos, end_pos, clean_title), ...]，按文本位置升序排列。
    """
    headings = _collect_heading_matches(
        text,
        [en_line, en_mid],
        dedup_distance,
    )
    return _expand_continuation_lines(text, headings, heading_number_regex)
