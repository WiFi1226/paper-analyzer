...
class _PromptCache:
    ...
    def write_meta(
        ...
        (self.prompts_root / "_meta.json").write_text(
...
def _run(args: argparse.Namespace, config: Config) -> None:
    ...
    if cache:
        ...
        kept_files.add(str((cache.prompts_root / "_meta.json").resolve()))
    ...
    if args.output:
        ...
        assert cache is not None  # paper_name 存在时 cache_dir 一定会被推断
