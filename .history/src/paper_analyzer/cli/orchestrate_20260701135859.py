...
def _resolve_input(user_input: str, config: Config) -> tuple[Path, Path | None]:
...
def _run(args: argparse.Namespace, config: Config) -> None:
    ...
    txt_path, pdf_path = _resolve_input(args.file, config)
    paper_name = txt_path.stem

    sections, sections_json_path = _run_split(txt_path, config)
