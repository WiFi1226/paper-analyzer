...
用法:
    paper-assemble my_paper
    paper-assemble my_paper --outputs-dir _contents/ -o report.md
"""
...
def main() -> None:
    ...
    parser.add_argument("paper_name", help="论文名称（用于标题和默认输出路径）")
    parser.add_argument("-o", "--output", default=None,
                        help="输出路径（默认 output/<论文名>/<论文名>_analysis.md）")
    parser.add_argument("--outputs-dir", default=None,
                        help="agent 输出目录（默认 output/<论文名>/cache/_contents/）")
...
def _run(args: argparse.Namespace, config: Config) -> None:
    ...
    order = get_agent_canonical_order(config)
    label = get_agent_section_map(config)
    ...
    # 2. 确定输出目录
    out_dir = Path(args.outputs_dir) if args.outputs_dir else config.outputs_dir(paper_name)
    ...
    # 5. 输出
    out_path = Path(args.output) if args.output else config.analysis_path(paper_name)
