...
from paper_analyzer.cache import (
    compute_rules_hash, compute_sections_hash,
...
class _PromptCache:
    ...
    封装所有磁盘 I/O：读取 _meta.json、查找缓存条目、指纹校验、
    从文件恢复 prompt、写入 prompt/content 文件、写入元数据、清理孤儿文件。
    """

    def __init__(self, cache_dir: Path):
        self.cache_dir = cache_dir
        self.prompts_dir = cache_dir / "_prompts"
        self.prompts_dir.mkdir(parents=True, exist_ok=True)

    # ── 读 ──────────────────────────────────────────────────────

    def load_meta(self) -> dict | None:
        """读取 _meta.json，不存在时返回 None。"""
        return load_existing_meta(self.cache_dir)
    ...
    def restore_parts(self, entry: dict, max_chars: int) -> list[dict[str, Any]] | None:
        ...
        for part in entry.get("parts", []):
            ...
            truncated = _truncate_if_oversized(prompt_text, file_path, max_chars)
    ...
    def write_prompt(self, p: dict[str, Any]) -> str:
        ...
        if p.get("content_text"):
            content_fpath = self.prompts_dir / fname.replace("prompt_", "content_")
    ...
    def write_meta(
        ...
        (self.prompts_dir / "_meta.json").write_text(
    ...
    def cleanup(self, kept_files: set[str]) -> None:
        """清理 cache 目录下的孤儿文件。"""
        cleanup_orphaned_files(self.cache_dir, kept_files)
...
def _run(args: argparse.Namespace, config: Config) -> None:
    ...
    existing_agent_entries = cache.load_meta().get("agent_entries", []) if cache else []
    ...
    for agent, titles in agent_titles.items():
        ...
        if cached_entry and cache.is_fresh(cached_entry, rules_hash, sections_hash):
        ...
        for p in agent_prompts:
            ...
            p["prompt_text"] = _truncate_if_oversized(p["prompt_text"], file_path, max_chars)
    ...
    if cache:
        ...
        kept_files.add(str((cache.prompts_dir / "_meta.json").resolve()))
