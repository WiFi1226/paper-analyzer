...
def _run(args: argparse.Namespace) -> None:
    ...
    for agent in order:
        ...
        if fp.exists():
            ...
            continue
        # 检查分片文件（{agent}_part*.md）
        part_files = sorted(out_dir.glob(f"{agent}_part*.md"))
        if part_files:
            content = [pf.read_text(encoding="utf-8") for pf in part_files]
            outputs[agent] = "\n\n---\n\n".join(content)
            parts_info[agent] = len(part_files)
