...
class RoutingData:
    ...
    def sort_by_agent_order(self, agent_order_map: dict[str, int]) -> None:
        """按 agent.order 值重排章节（用于终端展示）。

        排序规则：
          已分配章节  → 按 agent.order 升序，同 order 按 _index
          未分配章节  → order=9998，按 _index
          跳过章节    → order=9999，按 _index
        """
        def _sort_key(s: Section) -> tuple[int, int]:
            if s.skipped:
                return (9999, s._index)
            if not s.agents:
                return (9998, s._index)
            min_order = min(agent_order_map.get(a, 9997) for a in s.agents)
            return (min_order, s._index)

        self.sections.sort(key=_sort_key)
...
def _build_agent_order_map(agents: list[dict]) -> dict[str, int]:
    """构建 agent 内部名 → order 映射，用于章节排序。"""
    return {a["name"]: a["order"] for a in agents}
...
def _run(args: argparse.Namespace, config: Config) -> None:
    ...
    agent_order_map = _build_agent_order_map(agents)
    agent_label_map = _build_agent_label_map(agents)

    if len(agents) > 26:
        logger.warning("Agent 数量超过 26 个，仅前 26 个分配快捷键")

    # 3. 按 agent.order 重排章节（问题 1 修复）
    routing_data.sort_by_agent_order(agent_order_map)
