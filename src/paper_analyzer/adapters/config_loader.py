...
def _load_with_fallback(
    ...
    validator_args: tuple[Any, ...] | None = None,
) -> dict[str, Any]:
    ...
    data = load_yaml(path)
    issues = validator(data, *(validator_args or ()))
...
def load_routing(config: Config | None = None) -> dict[str, Any]:
    ...
    return _load_with_fallback(
        config, "routing.yaml", _validate_routing, (agent_names,),
    )
