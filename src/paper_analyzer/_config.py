...
class Config:
    ...
    def sections_path(self, paper_name: str) -> Path:
        """返回章节 JSON 路径：output/<论文名>/cache/<论文名>_sections.json"""
        return self.cache_dir(paper_name) / f"{paper_name}_sections.json"

    def routing_path(self, paper_name: str) -> Path:
        """返回路由方案路径：output/<论文名>/cache/_routing.json"""
        return self.cache_dir(paper_name) / "_routing.json"
    ...
    def outputs_dir(self, paper_name: str) -> Path:
        """返回 agent 输出目录：output/<论文名>/cache/_outputs/。自动创建。"""
        d = self.cache_dir(paper_name) / "_outputs"
        d.mkdir(parents=True, exist_ok=True)
        return d
