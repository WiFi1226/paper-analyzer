...
# Agent 分类显示顺序（shortcut 面板排列用）
_CATEGORY_ORDER = {
    "paper_structure": 0,
    "empirical_methodology": 1,
    "results_reporting": 2,
}
...
def _load_agent_list(config: Config) -> list[dict[str, Any]]:
    ...
    result.sort(key=lambda a: (_CATEGORY_ORDER.get(a["category"], 99), a["order"]))
    return result


def _build_digit_map(agents: list[dict]) -> dict[str, str]:
    """为 Agent 分配数字键标签。
    ...
    Returns:
        {agent_name: display_label}
    """
    label_map: dict[str, str] = {}
    for i, a in enumerate(agents):
        if i < 9:
            label_map[a["name"]] = f"[{i + 1}]"
        else:
            label_map[a["name"]] = f"[0{i - 8}]"  # 索引9→[01], 10→[02]...
    return label_map
...
def _build_tui_app(
    ...
    cursor_index = 0
    _digit_prefix = False  # True=按了 0，等待第二位数字
    ...
    style = Style.from_dict({
        ...
        "digit-prefix": "bg:#ff6600 bold #ffffff",
    ...
    # ── 数字键 toggle agent ──
    # 1-9 = 立即 toggle agent 1-9
    # 0 = 十位前缀，再按 1-9 = agent 10-18

    @kb.add("0")
    def _on_zero(event: Any) -> None:
        """按下 0：进入十位前缀模式；再按一次退出。"""
        nonlocal _digit_prefix
        _digit_prefix = not _digit_prefix

    for i in range(1, 10):
        ch = str(i)
        @kb.add(ch)
        def _on_digit(event: Any, digit: str = ch) -> None:
            nonlocal cursor_index, _digit_prefix
            if _digit_prefix:
                _digit_prefix = False
                idx = 9 + int(digit) - 1  # 0+1→索引9, 0+2→索引10...
            else:
                idx = int(digit) - 1  # 1→索引0, 2→索引1...

            if routing_data.sections and 0 <= idx < len(agents):
                sec = routing_data.sections[cursor_index]
                sec.skipped = False
    ...
    @staticmethod
    def _rpad_visual(text: str, width: int) -> str:
        """将文本右对齐填充到指定视觉宽度。"""
        text_width = wcswidth(text)
        if text_width >= width:
            return text
    ...
    def _render_table() -> list[tuple[str, str]]:
        ...
        for i, sec in enumerate(routing_data.sections):
            ...
            if wcswidth(raw_title) > 31:
                # 逐字符截断，直到视觉宽度 <= 31
                for cut in range(len(raw_title), 0, -1):
                    if wcswidth(raw_title[:cut] + "…") <= 31:
                        title_display = raw_title[:cut] + "…"
                        break

            row = (
                f" {prefix}{i + 1:<2}  "
                f"{_pad_visual(title_display, 32)}  "
                f"{sec.char_count:>6,}  "
            )
    ...
    def _render_shortcuts() -> list[tuple[str, str]]:
        ...
        # ── 十位前缀等待提示 ──
        if _digit_prefix:
            lines.append(("class:digit-prefix",
                          " 【按了 0，等待输入第二位数字 (1-9)】\n"))

        hint = "（0+数字=10号以后）" if len(agents) > 9 else ""
        lines.append(("class:header",
                       f"Agent 快捷键（按数字键切换选中/取消）{hint}:\n"))
        ...
        for cat, cat_agents in by_category.items():
            ...
            for a in cat_agents:
                label = digit_label_map.get(a["name"], "[?]")
                ...
                cat_parts.append((style_key, f"{label}{a['section_label']}  "))
...
def _run(args: argparse.Namespace, config: Config) -> None:
    ...
    digit_label_map = _build_digit_map(agents)
    agent_label_map = _build_agent_label_map(agents)

    if len(agents) > 18:
        logger.warning("Agent 数量超过 18 个，超出的部分无法通过 0+数字键访问")
    ...
    # 4. 启动 TUI
    app = _build_tui_app(
        routing_data, agents, digit_label_map, agent_label_map, routing_path,
    )
    result = app.run()
